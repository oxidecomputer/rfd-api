...
impl PermissionStorage for Permissions<ApiPermission> {
    fn contract(&self, user: &impl UserPermissionHolder) -> Self {
        ...
        for p in self.iter() {
            match p {
                ApiPermission::GetRfd(number) => { rfds.insert(*number); },
                ApiPermission::GetDiscussion(number) => { discussions.insert(*number); },
                ApiPermission::GetOAuthClient(id) => { read_oauth_clients.insert(*id); },
                ApiPermission::UpdateOAuthClient(id) => { update_oauth_clients.insert(*id); },
                ApiPermission::DeleteOAuthClient(id) => { delete_oauth_clients.insert(*id); },

                ApiPermission::GetApiUser(id) => {
                    contracted.push(if id == user.id() {
